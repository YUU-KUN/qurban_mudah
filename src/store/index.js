  ...
  actions: {
    login({commit}, petugas){
      return new Promise((resolve, reject) => {
        commit('auth_request')
        axios.post('/petugas/login', petugas).then(response => {
            const token = response.data.token
            commit('auth_success', token)
            localStorage.setItem('Role', response.data.petugas.level)
            localStorage.setItem('Name', response.data.petugas.nama_petugas)
